...
from pipeline_cogvideox_image_to_video_mask import CogVideoXImageToVideoMaskPipeline
...
class CustomCogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        if self.place_in_transformer in self.attnstore.layer_in_use:
            attn_weight = attn.get_attention_scores(query.flatten(0, 1)[:, 226:226+1350], key.flatten(0, 1)) # first frame image of the video
            attn_weight = attn_weight.view(batch_size, -1, 1350, 17776)[:, :, :, [5]] # token idx of interest
            attn_weight = attn_weight.mean(dim=1) # average over all heads
            self.attnstore(attn_weight, True, self.place_in_transformer)

            # attn_weight_ = attn.get_attention_scores(query.flatten(0, 1)[:, [5]], key.flatten(0, 1)) # first frame image of the video
            # attn_weight_ = attn_weight_.view(batch_size, -1, 1, 17776)[:, :, :, 226:226+1350] # token idx of interest
            # attn_weight_ = attn_weight_.transpose(-2, -1).mean(dim=1) # average over all heads

            # self.attnstore(attn_weight_, True, self.place_in_transformer)
...
class AttentionStore(AttentionControl):
    @staticmethod
    def get_empty_store():
        return {f"transformer_blocks.{i}.attn.processor": [] for i in range(42)}
        # return {"down_cross": [], "mid_cross": [], "up_cross": [], "down_self": [], "mid_self": [], "up_self": []}

    def forward(self, attn, is_cross: bool, place_in_transformer: str):
        key = f'transformer_blocks.{place_in_transformer}.attn.processor'
        # if attn.shape[1] <= 32**2:  # avoid memory overhead
        self.step_store[key] = [attn]
        return attn
...
def register_attention_control(model, controller):
    ...
    for name in model.attn_processors.keys():
        ...
        cross_att_count += 1
        attn_procs[name] = CustomCogVideoXAttnProcessor2_0(attnstore=controller, place_in_transformer=block_num)
...
def log_validation(
    ...
    pipe: CogVideoXImageToVideoMaskPipeline,
    args: Dict[str, Any],
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "wandb":
            ...
            tracker.log(
                {
                    phase_name: [
                        wandb.Video(filename, caption=f"{i}: {name}")
...
def run_validation(
    ...
    pipe = CogVideoXImageToVideoMaskPipeline.from_pretrained(
    ...
    extra_tokens = "The person interacts with <TGT> object."
    ...
    for validation_image, validation_prompt in zip(validation_images, validation_prompts):
        pipeline_args = {
            ...
            "masks": [load_image(validation_image.replace("images", "masks_hum")), load_image(validation_image.replace("images", "masks_obj"))],
            "prompt": extra_tokens + validation_prompt,
...
class CollateFunction:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        prompts_extra_tokens = [x["prompt_extra_tokens"] for x in data[0]]

        if self.load_tensors:
            prompts = torch.stack(prompts).to(dtype=self.weight_dtype, non_blocking=True)
            prompts_extra_tokens = torch.stack(prompts_extra_tokens).to(dtype=self.weight_dtype, non_blocking=True)

        images = [x["image"] for x in data[0]]
        images = torch.stack(images).to(dtype=self.weight_dtype, non_blocking=True)

        masks_hum = [x["mask_hum"] for x in data[0]]
        masks_hum = torch.stack(masks_hum).to(dtype=self.weight_dtype, non_blocking=True)

        masks_obj = [x["mask_obj"] for x in data[0]]
        masks_obj = torch.stack(masks_obj).to(dtype=self.weight_dtype, non_blocking=True)
        ...
        return {
            ...
            "videos": videos,
            "prompts": prompts,
            "prompts_extra_tokens": prompts_extra_tokens,
            "masks_hum": masks_hum,
            "masks_obj": masks_obj,
        }
...
def main(args):
    ...
    # tokenizer.add_tokens(["<SRC>", "<TGT>"])
    # tokenizer.add_tokens(["<MSK1>", "<MSK2>"])
    # text_encoder.resize_token_embeddings(len(tokenizer))

    # extra_tokens = "<SRC> person interacts with <TGT> object. " 0th, 5th tokens are <SRC> and <TGT> respectively [32100, 568, 6815, 7, 28, 32101, 3735, 5]
    # extra_tokens = "<MSK1> interacts with <MSK2>. " 0th, 4th tokens are <MSK1> and <MSK2> respectively [32100, 6815, 7, 28, 32101, 3, 5]
    # extra_tokens = "<MSK1> entity interacts with <MSK2> object. " 0th, 1st, 5th, 6th tokens [32100, 10409, 6815, 7, 28, 32101,  3735, 5]
    # extra_tokens = "The entity interacts with <TGT> object. " 5th token [37, 10409,  6815,     7,    28, 32100,  3735,     5]
    # extra_tokens = "The person interacts with target object. " 5th token [37,   568,  6815,     7,    28,  2387,  3735,     5]
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            CogVideoXImageToVideoMaskPipeline.save_lora_weights(
                output_dir,
    ...
    def load_model_hook(models, input_dir):
        ...
        lora_state_dict = CogVideoXImageToVideoMaskPipeline.lora_state_dict(input_dir)
    ...
    # Modify Image projection Conv2D layer of transformer to take additional mask inputs
    with torch.no_grad():
        new_conv_in = torch.nn.Conv2d( # 1 for object mask
            transformer.patch_embed.proj.in_channels + 1, transformer.patch_embed.proj.out_channels, \
    ...
    save_args.update(
        {
            ...
            "num_batches_each_epoch": len(train_dataloader),
            "num_epochs": args.num_train_epochs,
            "instantaneous_batch_size_per_device": args.train_batch_size,
    ...
    if args.use_cross_attention_loss:
        attention_store = AttentionStore()
        register_attention_control(transformer, attention_store)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                masks_hum = batch["masks_hum"][:, :, 0].to(accelerator.device, non_blocking=True) # RGB to L
                masks_obj = batch["masks_obj"][:, :, 0].to(accelerator.device, non_blocking=True) # RGB to L
                prompts = batch["prompts"]
                prompts_extra_tokens = batch["prompts_extra_tokens"]
                ...
                masks_hum = F.interpolate(masks_hum, size=(video_latents.shape[3], video_latents.shape[4]), mode="nearest").unsqueeze(1)
                masks_obj = F.interpolate(masks_obj, size=(video_latents.shape[3], video_latents.shape[4]), mode="nearest").unsqueeze(1)
                # masks_both = torch.cat([masks_hum, masks_obj], dim=2) # channel-wise concat
                mask_padding_shape = (video_latents.shape[0], video_latents.shape[1] - 1, *masks_obj.shape[2:])
                mask_latent_padding = image_latents.new_zeros(mask_padding_shape)
                masks_both = torch.cat([masks_obj, mask_latent_padding], dim=1)

                if random.random() < args.noised_image_dropout:
                    image_latents = torch.zeros_like(image_latents)

                # use_zero_mask = random.random() < args.noised_image_dropout
                # if use_zero_mask:
                #     masks_both = torch.zeros_like(masks_both)
                ...
                if not args.load_tensors:
                    ...
                    prompt_embeds = prompts_extra_tokens.to(dtype=weight_dtype)
                ...
                noisy_model_input = torch.cat([noisy_video_latents, image_latents, masks_both], dim=2)
                ...
                # layer to use for cross-attention loss
                if args.use_cross_attention_loss:
                    attention_store.step_store = attention_store.get_empty_store()
                    # attention_store.layer_in_use = random.sample(range(42), 7)
                    # attention_store.layer_in_use = [0, 7, 14, 21, 28, 35, 41]
                    attention_store.layer_in_use = [5, 8, 11, 14, 17, 20, 23]
                    # attention_store.layer_in_use = [i for i in range(42)]
                    # attention_store.token_idx = [5] torch.where(token_idx[0] == 32101)[0].tolist()
                    # attention_store.layer_in_use = [9, 14, 19]
                    # attention_store.layer_in_use = [10, 6, 8, 7, 18, 13, 11]
                    # attention_store.layer_in_use = [10, 6, 8, 7, 18]
                    # attention_store.layer_in_use = [14, 21, 28]
                ...
                if args.use_cross_attention_loss:
                    gt_mask = masks_both[:, 0]
                    gt_mask = F.interpolate(gt_mask, size=(30, 45), mode="nearest")

                    cross_attention_mask = None
                    cross_attention_mask = [v[0] for k, v in attention_store.step_store.items() if len(v) > 0]
                    cross_attention_mask = torch.stack(cross_attention_mask, dim=0)
                    cross_attention_mask = cross_attention_mask.mean(dim=0)
                    cross_attention_mask = cross_attention_mask.permute(0, 2, 1).reshape(-1, 1, 30, 45).to(gt_mask) # [B, num_tokens, H, W]
                    cross_attention_mask_norm = (cross_attention_mask - cross_attention_mask.min()) / (cross_attention_mask.max() - cross_attention_mask.min() + 1e-6)
            
                    cross_attention_mask_loss = args.lambda_cross_attention_loss * F.mse_loss(cross_attention_mask_norm, gt_mask.repeat(1, 1, 1, 1)) # [B, num_tokens, H, W]
                    loss += cross_attention_mask_loss
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process or accelerator.distributed_type == DistributedType.DEEPSPEED:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        # save conv2d weights
                        torch.save(transformer.patch_embed.proj.state_dict(), os.path.join(args.output_dir, f"patch_embed_conv2d.pth"))
                ...
                logs.update(
                    {
                        ...
                        "cross_attention_mask_loss": cross_attention_mask_loss.detach().item(),
                    }
                )
